package com.example.avaliadorimovel.details.utils;
...
public class SpinnerLayout extends RelativeLayout {
    ...
    private static SparseArray<SpinnerMode> mFooterModeSparse = new SparseArray<>();
    private static SparseIntArray mSpinnerGravitySparse = new SparseIntArray();

    static {
        mFooterModeSparse.put(0, SpinnerMode.MODE_TRANSLATE);
        mFooterModeSparse.put(1, SpinnerMode.MODE_EXPAND);
    ...
    private void reactionOfBackgroundWhenToOpen(boolean backSurfaceAvailable) {
        if (backSurfaceAvailable) {
            if (getLayoutParams() instanceof LayoutParams) {
                mOriginRootLayoutParams = (LayoutParams) getLayoutParams();
